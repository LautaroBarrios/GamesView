export function validate(inputs) {
  ...
  if (inputs.name.length > 70) error.name = "Maximum of 70 characters";
  ...
  if (inputs.description.length > 650)
    error.description = "Description exceeds the 650 character limit";

  if (inputs.description !== "" && !notSimbolsPuations.test(inputs.description))
    error.description = "Description cannot contain symbols";
